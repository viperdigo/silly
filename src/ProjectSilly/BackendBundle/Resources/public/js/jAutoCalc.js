...
(function ($) {
    $.fn.jAutoCalc = function (method) {
        ...
        var funcs = {
            'sum': {
                rgx: 'sum\\({([^}]+)}\\)', exec: function (field, ctx, numberFormat) {
                    m = 0;
            ...
            'avg': {
                rgx: 'avg\\({([^}]+)}\\)', exec: function (field, ctx, numberFormat) {
                    m = 0;
            ...
            'min': {
                rgx: 'min\\({([^}]+)}\\)', exec: function (field, ctx, numberFormat) {
                    return Math.min.apply(this, $(getFieldSelector(field), ctx).map(function (i, e) {
                        return numCleanse($(e).val(), numberFormat)
                    }).get());
                }
            },
            'max': {
                rgx: 'max\\({([^}]+)}\\)', exec: function (field, ctx, numberFormat) {
            ...
            'count': {
                rgx: 'count\\({([^}]+)}\\)', exec: function (field, ctx) {
                    return $(getFieldSelector(field), ctx).length
                }
            },
            'countNotEmpty': {
                rgx: 'countNotEmpty\\({([^}]+)}\\)', exec: function (field, ctx) {
        ...
        var ops = {
            '+': {
                op: '+', precedence: 10, assoc: 'L', exec: function (l, r) {
                    return l + r;
                }
            },
            '-': {
            ...
            '*': {
                op: '*', precedence: 20, assoc: 'L', exec: function (l, r) {
                    return l * r;
                }
            },
            '/': {
            ...
            '**': {
                op: '**', precedence: 30, assoc: 'R', exec: function (l, r) {
                    return Math.pow(l, r);
                }
            }
        };
